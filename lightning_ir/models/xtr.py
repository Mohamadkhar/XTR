...
class XTRConfig(MultiVectorBiEncoderConfig):
    """
    XTRConfig is a configuration class for the XTR model, inheriting from MultiVectorBiEncoderConfig.
    It is used to define the configuration parameters specific to the XTR model.
    """
    model_type = "xtr"
...
class XTRModel(MultiVectorBiEncoderModel):
    ...
    def compute_token_retrieval_mask(
        ...
        k: int = None, # self.config.k_train,
    ) -> torch.Tensor:
        """
        Berechnet die Token-Retrieval-Maske Â_ij nach der XTR-Methodik.
        
        Â_ij = [j ∈ top-k_{j'}(P_{ij'})] wo j' über Token aus ALLEN Dokumenten im Mini-Batch läuft.
        
        Args:
            query_embeddings: [batch_size_q, query_len, embed_dim]
            doc_embeddings: [batch_size_d, doc_len, embed_dim] 
            scoring_mask_query: [batch_size_q, query_len]
            scoring_mask_doc: [batch_size_d, doc_len]
            k: Anzahl der Top-k Token (default: self.config.k_train)
            
        Returns:
            top_k_mask: [batch_size_q, query_len, batch_size_d, doc_len] - Alignment-Maske
        """
        if k is None:
           k = getattr(self.config, 'k_train', 10)
           
            
        batch_size_q, query_len, embed_dim = query_embeddings.shape
        batch_size_d, doc_len, _ = doc_embeddings.shape
        
        #Verarbeitung für große Similarity-Matrizen
        #max_tokens = batch_size_q * query_len * batch_size_d * doc_len
        #if max_tokens > 1e8:  # Threshold für Speicheroptimierung
        # Implementiere batch-weise Top-K Berechnung
        #  pass
        #sichergehen, dass k nicht größer ist als die Gesamtzahl der Dokument-Token
        if k <= 0:
            return torch.zeros(batch_size_q, query_len, batch_size_d, doc_len, dtype=torch.bool, device=query_embeddings.device) 
         
        total_doc_tokens = batch_size_d * doc_len
        k = min(k, total_doc_tokens)

            
        query_flat = query_embeddings.view(-1, embed_dim)
        doc_flat = doc_embeddings.view(-1, embed_dim)
        
        
        """
        Gibt binäre Maske Â_ij zurück: True, wenn d_j unter den Top-k der Ähnlichkeiten zu q_i ist.
        """
        
        similarity = torch.matmul(query_flat, doc_flat.T)  
        similarity = similarity.view(batch_size_q, query_len, batch_size_d, doc_len)
        
        
        
        
        # Wenn scoring_mask_query oder scoring_mask_doc None sind, wird die gesamte Ähnlichkeit verwendet
        if scoring_mask_query is not None:
            query_mask_expanded = scoring_mask_query.unsqueeze(2).unsqueeze(3)
            similarity = similarity.masked_fill(~query_mask_expanded, float('-inf'))
        if scoring_mask_doc is not None:
            doc_mask_expanded = scoring_mask_doc.unsqueeze(0).unsqueeze(1)
            similarity = similarity.masked_fill(~doc_mask_expanded, float('-inf'))
           
           
        similarity_flattened = similarity.view(batch_size_q, query_len, -1)
        top_k_values, top_k_indices = torch.topk(similarity_flattened, k, dim=-1)
        threshold = top_k_values[..., -1:].unsqueeze(-1)
       # k_per_doc = min(k, batch_size_d * doc_len)
       # similarity_for_topk = similarity.view(batch_size_q, query_len, -1)
            
        top_k_values, top_k_indices = similarity.topk(k, dim=-1) #similarity_for_topk.topk(k, dim=-1)
        threshold = top_k_values[..., -1:].unsqueeze(-1)    
        #threshold = threshold.unsqueeze(-1) 
        
        #top_k_mask = similarity >= threshold
        #top_k_similarity = similarity.topk(k, dim=-1)
        #cut_off_similarity = top_k_similarity.values[..., [-1]]
        #top_k_mask = similarity >= cut_off_similarity
        if self.config.strict_top_k:
            top_k_mask = torch.zeros_like(similarity_flattened, dtype=torch.bool)
            for b in range(batch_size_q):
                for q in range(query_len):
                    top_k_mask[b, q, top_k_indices[b, q]] = True
            top_k_mask = top_k_mask.view(batch_size_q, query_len, batch_size_d, doc_len)
        else:
            top_k_mask = similarity >= threshold

        return top_k_mask # == Â_ij

class XTRTokenizer(BiEncoderTokenizer):
    """
    XTRTokenizer is a specialized tokenizer for the XTR model, inheriting from BiEncoderTokenizer.
    It is designed to handle the specific tokenization needs of the XTR model.
    """
    config_class = XTRConfig 
    def __init__(
            self,
